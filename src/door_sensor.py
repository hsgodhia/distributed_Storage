import Pyro4, Pyro4.naming, threading, time, sys, socket, traceback
...
class Door_sensor(DefaultSensor, RealProcess):
	def __init__(self, stype, name, comtype):
		...
		#continous stream of event simulation
		t = threading.Thread(target = self.gameloop)
		t.setDaemon(True)
		t.start()

	def gameloop(self):
		#eaech message carries the below payload, counter as its data
		i = 2000
		while 1:
			time.sleep(20)
			...
			i += 1
			st = time.time()
			msg = Message(i, MsgType.STATE, self.id, ts=self.getNextRealts())
			try:
	...
	#toggle the change loop on and off
